...
export default function App({ session }) {
  ...
  const exportPriceList = (format) => {
    ...
    (activePL.includedBuilds || []).filter(k => data[k]).forEach(k => {
      ...
      const cat = data[k].category || data[k].summary?.category || "Uncategorized";
      if (!byCategory[cat]) byCategory[cat] = [];
      byCategory[cat].push({ name: data[k].name, sku: k, description: data[k].description || "", sellPrice: e.hasOverride ? e.totalSellNative : e.minSellNative, type: "Build" });
    });
    (activePL.includedFG || []).forEach(idx => {
      const item = fgItems[idx]; if (!item) return;
      const c = calcFGCost(item, gs);
      const fgOvr = activePL.fgOverrides?.[String(idx)]?.sellPrice ?? null;
      const baseSell = fgOvr ?? (c.actualCAD != null ? c.actualCAD * (activePL.fxRate || 1) : null);
      const ship = (activePL.shippingAbsorbed ?? false) ? 0 : (activePL.shippingDefault ?? 0);
      const total = baseSell != null ? baseSell + ship : null;
      const cat = item.category || "Uncategorized";
      if (!byCategory[cat]) byCategory[cat] = [];
      byCategory[cat].push({ name: item.name, sku: item.sku || "", description: "", sellPrice: total, type: "FG" });
    });
    
    // Build CSV (simple Excel-compatible format)
    let csv = "\uFEFF"; // BOM for Excel UTF-8
    csv += `${activePL.name} - Price List\n`;
    csv += `Currency: ${activePL.currency} | Generated: ${new Date().toLocaleDateString()}\n`;
    csv += `\n`;
    
    const cats = Object.keys(byCategory).sort((a, b) => a === "Uncategorized" ? 1 : b === "Uncategorized" ? -1 : a.localeCompare(b));
    cats.forEach(cat => {
      csv += `\n${cat}\n`;
      csv += `Item,SKU,Description,Price (${activePL.currency})\n`;
      byCategory[cat].forEach(item => {
        const name = item.name.replace(/,/g, " ");
        const desc = item.description.replace(/,/g, " ");
        const price = item.sellPrice != null ? item.sellPrice.toFixed(2) : "";
        csv += `${name},${item.sku},${desc},${price}\n`;
      });
    });
    ...
    a.href = url;
    a.download = `${activePL.name.replace(/[^a-zA-Z0-9]/g, "_")}_PriceList_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
  ...
  const [editingFGOverride, setEditingFGOverride] = useState(null);

  const saveFGOverride = (plIdx, fgIdx, sellPrice) => {
    const next = JSON.parse(JSON.stringify(priceLists));
    if (!next[plIdx].fgOverrides) next[plIdx].fgOverrides = {};
    const key = String(fgIdx);
    if (sellPrice === null || sellPrice === "") delete next[plIdx].fgOverrides[key];
    else next[plIdx].fgOverrides[key] = { sellPrice: parseFloat(sellPrice) };
    persistPL(next); setEditingFGOverride(null); showToast("Price override saved");
  };
  ...
  const duplicatePriceList = (idx, newName, newCurrency) => {
    ...
    const newFGOverrides = {};
    Object.entries(src.fgOverrides || {}).forEach(([k, v]) => {
      if (v.sellPrice) newFGOverrides[k] = { sellPrice: Math.round(v.sellPrice * conversionFactor * 100) / 100 };
    });
    const newPL = { ...JSON.parse(JSON.stringify(src)), id: `pl_${Date.now()}`, name: newName || `${src.name} (Copy)`, currency: newCurrency || src.currency, fxRate: newFx, systemOverrides: newOverrides, fgOverrides: newFGOverrides };
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: THEME.bg, minHeight: "100vh", color: THEME.text }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(380px, 1fr))", gap: 20 }}>
            {priceLists.map((pl, i) => {
              ...
              (pl.includedBuilds || []).filter(k => data[k]).forEach(k => {
                ...
                if (e.hasOverride && e.totalSellNative) { totalRevenue += e.totalSellNative; }
                else { missingPrice++; }
              ...
              (pl.includedFG || []).forEach(idx => {
                const item = fgItems[idx]; if (!item) return;
                const c = calcFGCost(item, gs);
                const fgOvr = pl.fgOverrides?.[String(idx)]?.sellPrice ?? null;
                const sellNative = fgOvr ?? (c.actualCAD != null ? c.actualCAD * (pl.fxRate || 1) : null);
                const fgKpiShip = (pl.shippingAbsorbed ?? false) ? 0 : (pl.shippingDefault ?? 0);
                const totalSellKpi = sellNative != null ? sellNative + fgKpiShip : null;
                if (totalSellKpi != null) { totalRevenue += totalSellKpi; } else { missingPrice++; }
                const revCAD = totalSellKpi != null ? totalSellKpi / (pl.fxRate || 1) : null;
                const fgGM = revCAD != null && revCAD > 0 ? (revCAD - c.factoryCost) / revCAD : null;
                if (fgGM != null) { totalGM += fgGM; gmCount++; if (lowestGM === null || fgGM < lowestGM) { lowestGM = fgGM; lowestGMName = item.name; } }
              });
        ...
        <div style={{ padding: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <button className={`btn btn-sm ${showMarginAnalysis ? "btn-primary" : "btn-ghost"}`} onClick={() => setShowMarginAnalysis(!showMarginAnalysis)}>📊 Analysis</button>
              <button className="btn btn-green btn-sm" onClick={() => exportPriceList("excel")}>📥 Excel</button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16, padding: "12px 20px", background: THEME.surface, border: `1px solid ${THEME.border}`, borderRadius: 10 }}>
            <span style={{ fontSize: 13, color: THEME.textMuted, fontWeight: 500, whiteSpace: "nowrap" }}>Test Price Change:</span>
            <input type="range" min="-50" max="50" step="1" value={testDiscount} onChange={e => setTestDiscount(parseInt(e.target.value))} style={{ flex: 1, accentColor: testDiscount > 0 ? THEME.green : testDiscount < 0 ? THEME.red : THEME.accent }} />
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <input className="input-field" type="number" min="-100" max="100" step="1" value={testDiscount} onChange={e => setTestDiscount(Math.min(100, Math.max(-100, parseInt(e.target.value) || 0)))} style={{ width: 65, textAlign: "center", padding: "4px 8px" }} />
              <span style={{ color: THEME.textDim, fontSize: 13 }}>%</span>
            </div>
            <span style={{ fontSize: 12, fontWeight: 600, color: testDiscount > 0 ? THEME.green : testDiscount < 0 ? THEME.red : THEME.textDim, minWidth: 70, textAlign: "center" }}>{testDiscount > 0 ? `+${testDiscount}% ↑` : testDiscount < 0 ? `${testDiscount}% ↓` : "No change"}</span>
          </div>
          ...
          {showMarginAnalysis && (() => {
            ...
            (activePL.includedBuilds || []).filter(k => data[k]).forEach(k => {
              const e = calcPLBuildEntry(k, data[k], activePL, gs);
              if (e.hasOverride) {
                const discounted = e.totalSellNative * (1 + testDiscount / 100);
                ...
                items.push({ name: data[k].name, type: "Build", sellPrice: e.totalSellNative, discounted, factoryCost: e.factoryCost, origGM: e.realGM, discGM, discMargin, sym: e.sym });
              }
            });
            (activePL.includedFG || []).forEach(idx => {
              const item = fgItems[idx]; if (!item) return;
              const c = calcFGCost(item, gs);
              const fgOvr = activePL.fgOverrides?.[String(idx)]?.sellPrice ?? null;
              const fgBaseSell = fgOvr ?? (c.actualCAD != null ? c.actualCAD * (activePL.fxRate || 1) : null);
              if (fgBaseSell != null) {
                const maShipping = (activePL.shippingAbsorbed ?? false) ? 0 : (activePL.shippingDefault ?? 0);
                const sellNative = fgBaseSell + maShipping;
                const sym = CURRENCY_SYMBOLS[activePL.currency] || activePL.currency + " ";
            ...
            return (
              <div style={{ background: THEME.surface, border: `1px solid ${THEME.border}`, borderRadius: 10, padding: 20, marginBottom: 20 }}>
                <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, color: THEME.accent }}>📊 Margin Analysis at {testDiscount > 0 ? `+${testDiscount}%` : `${testDiscount}%`} Price Change</div>
                ...
                {items.filter(i => i.discGM != null && i.discGM < 0.2).length > 0 && (
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: THEME.orange, marginBottom: 8 }}>⚠ Items below 20% GM at {testDiscount > 0 ? `+${testDiscount}%` : `${testDiscount}%`}:</div>
                    <div className="table-wrap">
          ...
          {(() => {
            ...
            (activePL.includedBuilds || []).filter(k => data[k]).forEach(k => {
              ...
              allItems.push({
                ...
                sellPrice: e.hasOverride ? e.totalSellNative : null,
                baseSellNative: e.baseSellNative,
                totalSellNative: e.totalSellNative,
                margin: e.realMarginCAD,
                gm: e.realGM,
                sym: e.sym,
                hasOverride: e.hasOverride,
                shipping: e.shippingAbsorbed ? 0 : e.effectiveShipping,
              });
            });
            ...
            (activePL.includedFG || []).map(idx => {
              ...
              const sym = CURRENCY_SYMBOLS[activePL.currency] || activePL.currency + " ";
              const fgOvr = activePL.fgOverrides?.[String(idx)]?.sellPrice ?? null;
              const baseSellNative = fgOvr ?? (c.actualCAD != null ? c.actualCAD * fxRate : null);
              const fgShipping = (activePL.shippingAbsorbed ?? false) ? 0 : (activePL.shippingDefault ?? 0);
              const totalSellNative = baseSellNative != null ? baseSellNative + fgShipping : null;
              const revCAD = totalSellNative != null ? totalSellNative / fxRate : null;
              const ovrMargin = revCAD != null ? revCAD - c.factoryCost : null;
              const ovrGM = revCAD != null && revCAD > 0 ? ovrMargin / revCAD : null;
              allItems.push({
                type: "fg",
                key: `fg-${idx}`,
                fgIdx: idx,
                ...
                sellPrice: totalSellNative,
                baseSellNative: baseSellNative,
                totalSellNative: totalSellNative,
                margin: ovrMargin,
                gm: ovrGM,
                sym: sym,
                hasOverride: fgOvr != null,
                shipping: fgShipping,
              });
            });
                ...
                <div className="table-wrap" style={{ marginBottom: 20 }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, tableLayout: "fixed" }}>
                    ...
                    <thead><tr>
                      ...
                      {testDiscount !== 0 && <th style={{ ...headerStyle, textAlign: "right", color: testDiscount < 0 ? THEME.red : THEME.green }}>{testDiscount > 0 ? `+${testDiscount}%` : `${testDiscount}%`}</th>}
                      {testDiscount !== 0 && <th style={{ ...headerStyle, textAlign: "right", color: testDiscount < 0 ? THEME.red : THEME.green }}>GM% After</th>}
                    ...
                    <tbody>
                      {byCategory[cat].map(item => (
                        <tr key={item.key} className="row-hover" style={{ borderBottom: `1px solid ${THEME.border}` }}>
                          ...
                          {testDiscount !== 0 && (() => {
                            const sp = item.totalSellNative || item.sellPrice || item.minSell;
                          ...
                          <td style={{ ...colStyle, textAlign: "right", whiteSpace: "nowrap" }}>
                            ...
                            {item.type === "fg" && <button className="btn btn-ghost btn-sm" style={{ padding: "2px 8px", marginRight: 2, fontSize: 11 }} onClick={() => setEditingFGOverride({ plIdx: activePLIndex, fgIdx: item.fgIdx, fgName: item.name, current: activePL.fgOverrides?.[String(item.fgIdx)]?.sellPrice ?? "" })}>Set Price</button>}
      ...
      {managingPLItems !== null && priceLists[managingPLItems] && (<div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
        <div className="card" style={{ padding: 24, width: 700, maxWidth: "95vw", maxHeight: "85vh", overflowY: "auto" }}>
          ...
          <div style={{ maxHeight: 200, overflowY: "auto", border: `1px solid ${THEME.border}`, borderRadius: 8 }}>
            ...
            fgItems.map((item, origIdx) => ({ item, origIdx })).filter(({ item }) => !plSearch || item.name?.toLowerCase().includes(plSearch.toLowerCase()) || item.sku?.toLowerCase().includes(plSearch.toLowerCase())).map(({ item, origIdx }) => {
              const included = (priceLists[managingPLItems].includedFG || []).includes(origIdx);
              return (<div key={origIdx} onClick={() => toggleFGInPL(managingPLItems, origIdx)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", cursor: "pointer", borderBottom: `1px solid ${THEME.border}`, background: included ? "rgba(0,201,167,0.08)" : "transparent" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: THEME.textDim }}>{item.sku} · Cost: {fmtCurr(item.cost_cad)} {item.sell_price_cad ? `· Sell: ${fmtCurr(item.sell_price_cad)}` : ""}</div>
                </div>
              </div>);
            })}
          </div>
      ...
      {/* FG Price Override Modal */}
      {editingFGOverride && (<div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
        <div className="card" style={{ padding: 24, width: 400 }}>
          <h3 style={{ margin: "0 0 16px", fontSize: 16 }}>Set Sell Price Override</h3>
          <p style={{ fontSize: 13, color: THEME.textMuted, marginBottom: 12 }}>{editingFGOverride.fgName}</p>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", fontSize: 12, color: THEME.textDim, marginBottom: 3 }}>Sell Price ({priceLists[editingFGOverride.plIdx]?.currency})</label>
            <input className="input-field" type="number" step="0.01" defaultValue={editingFGOverride.current} id="fg_override_price" placeholder="Leave empty to use default" autoFocus />
          </div>
          <div style={{ fontSize: 11, color: THEME.textDim, marginBottom: 12 }}>Leave empty to use the default CAD sell price converted at the list's FX rate.</div>
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <button className="btn btn-ghost" onClick={() => setEditingFGOverride(null)}>Cancel</button>
            <button className="btn btn-primary" onClick={() => saveFGOverride(editingFGOverride.plIdx, editingFGOverride.fgIdx, document.getElementById("fg_override_price").value)}>Save</button>
          </div>
        </div>
      </div>)}
