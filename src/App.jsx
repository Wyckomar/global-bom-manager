          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(380px, 1fr))", gap: 20 }}>
            {priceLists.map((pl, i) => {
              ...
              (pl.includedFG || []).forEach(idx => {
                ...
                if (sellNative != null) { totalRevenue += sellNative; } else { missingPrice++; }
                const revCAD = sellNative != null ? sellNative / (pl.fxRate || 1) : null;
        ...
        <div style={{ padding: 24 }}>
          ...
          {showMarginAnalysis && (() => {
            ...
            (activePL.includedFG || []).forEach(idx => {
              ...
              const baseSellNative = fgOvr ?? (c.actualCAD != null ? c.actualCAD * (activePL.fxRate || 1) : null);
              if (baseSellNative != null) {
                const sellNative = baseSellNative;
          ...
          {(() => {
            ...
            (activePL.includedFG || []).map(idx => {
              ...
              const sym = CURRENCY_SYMBOLS[activePL.currency] || activePL.currency + " ";
              const fgOvr = activePL.fgOverrides?.[String(idx)]?.sellPrice ?? null;
              const sellNative = fgOvr ?? (c.actualCAD != null ? c.actualCAD * fxRate : null);
              const revCAD = sellNative != null ? sellNative / fxRate : null;
