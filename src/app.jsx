          ...
          {(activePL.includedBuilds || []).length > 0 && (<>
            ...
            {(() => {
              ...
              (activePL.includedBuilds || []).filter(k => data[k]).forEach(k => {
                ...
                if (!buildsByCategory[cat]) buildsByCategory[cat] = [];
                buildsByCategory[cat].push(k);
              });
              const cats = Object.keys(buildsByCategory).sort((a, b) => a === "Uncategorized" ? 1 : b === "Uncategorized" ? -1 : a.localeCompare(b));
              return cats.map(cat => (
                <PLCategorySection key={`bld-${cat}`} title={cat} count={buildsByCategory[cat].length} defaultOpen={true}>
                  <div className="table-wrap" style={{ marginBottom: 16 }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      <thead><tr style={{ borderBottom: `1px solid ${THEME.border}` }}>{["BUILD", "CATEGORY", "FACTORY (CAD)", `MIN SELL (${activePL.currency})`, `SELL (${activePL.currency})`, "MARGIN", "GM%", ""].map(h => (<th key={h} style={{ textAlign: ["BUILD", "CATEGORY", ""].includes(h) ? "left" : "right", padding: "10px 12px", color: THEME.textDim, fontWeight: 600, fontSize: 11 }}>{h}</th>))}</tr></thead>
                      <tbody>{buildsByCategory[cat].map(k => {
                        ...
                        return (<tr key={k} className="row-hover" style={{ borderBottom: `1px solid ${THEME.border}` }}>
                          ...
                          <td style={{ padding: "8px 12px", color: THEME.textDim, fontSize: 12 }}>{data[k].category || data[k].summary?.category || <span style={{ color: THEME.orange, cursor: "pointer" }} onClick={() => setEditBuildMeta({ key: k, name: data[k].name, description: data[k].description || "", category: data[k].category || data[k].summary?.category || "" })}>+ Add</span>}</td>
                          ...
                          <td style={{ padding: "8px 12px", textAlign: "right", color: e.hasOverride ? THEME.green : THEME.textMuted }}>{e.hasOverride ? fmtCurr(e.baseSellNative, e.sym) : "—"}</td>
                          ...
                          <td style={{ padding: "8px 12px", textAlign: "right" }}>
                            ...
                            <button className="btn btn-ghost btn-sm" style={{ padding: "2px 6px", color: THEME.red }} onClick={() => toggleBuildInPL(activePLIndex, k)} title="Remove">✕</button>
                          </td>
                        </tr>);
                      })}</tbody>
                    </table>
